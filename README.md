...
It is split into a NEST backend which serves as API for the frontend and the skribbltypo extension, and an Angular frontend whichreplaces the legacy skribbltypo website.  
...
The API uses rich swagger annotations to generate a meaningful openapi specs and the frontend client from that.  
There is also a [swagger interface](https://api.typo.rip/docs) available.
...
The frontend is live on the [www subdomain](https://www.typo.rip) and probably also accessible via redirect from the TLD https://typo.rip
